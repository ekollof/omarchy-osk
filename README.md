...
- **Socket**: `$XDG_RUNTIME_DIR/hypr-osk.sock`, one client (a new connect
  ...
  (root-owned, not group/world-writable). That is only a first filter:
  the compositor pins the installed omarchy-shell instance (the Wayland
  client that already owns mapped `omarchy-bar`) with a pidfd. While that
  pin is live, accept refuses any other process.
  `TEXT`/`KEY`/`MOD` additionally require that same instance to own a
  mapped `ekollof-osk` layer (visibility, not identity). Sends are
  non-blocking so a stalled client cannot pin the compositor or block
  plugin unload. `PMOVE`/`PBTN` always return `err pointer disabled`.
...
- **Gamepad**: auto-starts at plugin load. Prefers Steam Controller hidraw
  ...
  `gamescope` and exclusive-fullscreen games get the pad alone (hidraw/evdev
  closed); browsers / media players keep it. The OSK overlay reopens it.
...
- injection authority is a compositor-pinned pidfd of the session's
  omarchy-shell instance (the client that owns mapped `omarchy-bar`),
  not a caller-chosen layer namespace; a second quickshell mapping
  `ekollof-osk` is refused
